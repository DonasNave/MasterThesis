...
představuje staticky typovaný jazyk, který je založen na jazyce C++. Vznikl v roce 2000 a je nejpoužívanějších z jazyků pro vývoj aplikací na platformě .NET. C\# je objektově orientovaný jazyk, který umožňuje vytvářet aplikace pomocí tříd, objektů a metod.

\n{4}{F\#}

je funkcionální jazyk, který je založen na jazyce OCaml. Základem F\# je funkcionální programování, které umožňuje vytvářet aplikace pomocí funkcí. F\# je multiplatformní jazyk, který je možné použít na operačních systémech Windows, Linux a macOS. Vývojáři mohou využívat několik nástrojů pro vývoj aplikací v jazyce F\#. Mezi nejpoužívanější nástroje patří Visual Studio, Visual Studio Code a JetBrains Rider.

\n{4}{Visual Basic}

je založen na jazyce BASIC. Historie jazyka sahá až do roku 1964, kdy byl vytvořen první jazyk BASIC. 
...
Pro desktopové počítače je využíván framework .NET Core. .NET Core umožňuje vývoj desktopových aplikací pro platformy Windows, Linux a macOS pomocí jazyka C\# a .NET. Vývojáři mohou využívat nástroje a knihovny platformy .NET pro vývoj desktopových aplikací. .NET Core poskytuje sadu nástrojů pro vývoj desktopových aplikací, včetně nástrojů pro vytváření uživatelského rozhraní, správu dat a komunikaci s cloudovými službami. .NET Core umožňuje vývoj desktopových aplikací pro platformy Windows, Linux a macOS pomocí jednoho kódu. Vývojáři mohou vytvářet aplikace pro různé platformy pomocí jednoho kódu a sdílet kód mezi různými platformami.



\n{4}{Xamarin}

Pro vývoj mobilních aplikací je využíván framework Xamarin. Xamarin umožňuje vývoj mobilních aplikací pro platformy Android a iOS pomocí jazyka C\# a .NET. Vývojáři mohou využívat nástroje a knihovny platformy .NET pro vývoj mobilních aplikací. Xamarin poskytuje sadu nástrojů pro vývoj mobilních aplikací, včetně nástrojů pro vytváření uživatelského rozhraní, správu dat a komunikaci s cloudovými službami. Xamarin umožňuje vývoj mobilních aplikací pro platformy Android a iOS pomocí jednoho kódu. Vývojáři mohou vytvářet aplikace pro obě platformy pomocí jednoho kódu a sdílet kód mezi oběma platformami. Xamarin poskytuje sadu nástrojů pro vývoj mobilních aplikací, včetně nástrojů pro vytváření uživatelského rozhraní, správu dat a komunikaci s cloudovými službami. Xamarin umožňuje vývoj mobilních aplikací pro platformy Android a iOS pomocí jednoho kódu. Vývojáři mohou vytvářet aplikace pro obě platformy pomocí jednoho kódu a sdílet kód mezi oběma platformami. Xamarin poskytuje sadu nástrojů pro vývoj mobilních aplikací, včetně nástrojů pro vytváření uživatelského rozhraní, správu dat a komunikaci s cloudovými službami. Xamarin umožňuje vývoj mobilních aplikací pro platformy Android a iOS pomocí jednoho kódu.

Pro .NET MAUI aplikace je cílem kompilace vytvoření spustitelného souboru, který lze spustit na cílovém zařízení. To se liší od jiných typů aplikací, které mohou být spuštěny na různých zařízeních, jako jsou webové aplikace, desktopové aplikace a další. 
