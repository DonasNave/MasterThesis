...
\begin{itemize}
    ...
    \item \textbf{Heatmapy a Sankey diagramy} - Ilustrují složité vztahy a toky mezi komponentami systému.
\end{itemize}

Tyto vizuální nástroje umožňují týmům rychle identifikovat klíčová míst, jako jsou bottlenecks (úzká místa výkonu) n, a řešit potenciální problémy dříve, než ovlivní stabilitu systému nebo uživatelský dojem.
...
\begin{enumerate}
    ...
    \item \textbf{Nasazení služeb pro správu a kolekci dat} - Nasazení služeb pro správu a kolekci dat je zajištěno pomocí nástrojů, které jsou schopny zprostředkovat sběr dat z různých zdrojů a zároveň zajišťují jejich zpracování a zobrazení. Tímto je zajištěno, že data jsou zpracována a zobrazena v reálném čase.
    \item \textbf{Vizualizace dat} - Vizualizace dat je zajištěna pomocí nástrojů, které jsou schopny zobrazit data v uživatelsky přívětivé podobě. Tímto je zajištěno, že data jsou zobrazena v reálném čase a jsou přehledná a srozumitelná.
\end{enumerate}
