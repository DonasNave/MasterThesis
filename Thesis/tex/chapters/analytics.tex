...
\tab{Velikost výstupu programu AOT služeb pro různé OS}{tab:priklad}{1}{|l|c|c|c|r|}{
  \hline
...
Testování přineslo rozdílné výkonostní výsledky mezi JIT a AOT kompilací. Dle předpokladu AOT služby využívaly méně času CPU. Paměťová stopa však u nich byla větší. Konečně, AOT služby byly schopné v průměru rychleji odpovídat. Výsledek poukazuje na dodatečnou režii od CLR na CPU a IO v případě JIT služeb. Větší využití paměti u AOT služeb poukazuje na režii vygenerovaných typů a přítomnost dalších závislotí v programu načteného v paměti. Čistá rychlost zpracování požadavku a odpovědi není v mnoha případech kritickým faktorem. Avšak v případě velkého množství požadavků, může být rozdíl v řádech milisekund zásadní.
