...
Z pohledu microservice architektury má velký význam měřit velikost výstupního obrazu .NET služeb. Sestavení obrazu je závislé na prostředí, vyhodnocení a stažení závislostí, kompilaci a publikování aplikace. Co se týče vytvořených kontejnerů, s ohledem na podstatu zvolených technologií pro .NET služby, bylo možno využít \emph{trimming} i pro JIT kompilované služby. Tyto služby jsou v rámci obrazu nasazeny formou \emph{self-contained}, což znamená, že spolu s programem jsou vypublikovány i závislosti .NET runtime. Nepotřebné z těchto závislostí jsou procesem trimmingu očesány a výsledný obraz nabývá velmi malé velikosti. Následující tabulka zobrazuje velikosti obrazů služeb pro oba kompilační cíle. V případě JIT služeb je doplněna dvojí varianta, self-contained očesaný obraz a obecný obraz s plnohodnotným .NET runtime. Použitý příkaz k sestavení obrazu je \emph{docker build -t <service>:<tag> -f Dockerfile-<target> .}. V příkazu \emph{<target>} představuje vybranou kompilační metodu AOT nebo JIT a \emph{<service>} jméno služby. Konkrétní Dockerfile soubory jsou součástí zdrojů práce. 

\tab{Velikost obrazu služeb}{tab:priklad}{0.8}{|l|c|c|r|}{
  \hline
   Služba & JIT (MB) & JIT-trimmed (MB) & AOT (MB) \\ \hline
...
Výsledek poskytuje relativní potvrzení i vyvrácení hypotézy 2. Z pohledu služeb využívajících plnohodnotného .NET runtime, takto vytvořené konktejnery jsou výrazně větší než je tomu u kontejnerů nativních AOT služeb. Nicméně z pohledu implementace služeb splňujících definované požadavky a využívajících JIT kompilaci, představují self-contained varianty obrazu s trimming validní výstup. Bez ohledu na kompatibilitu plynoucí z podstaty práce a kódové báze služeb, použití funkce trimming s sebou nese určitá rizika a nejedná se o běžnou formu nasazení služby. S ohledem na prezentované výsledky lze deklarovat hypotézu 2 v plném znění jako nepravdivou.
