...
run_k6_test() {
    ...
    k6 run --out influxdb=http://localhost:8086/k6 scripts/scenario1.js -e SERVICE_URL=$url -e SERVICE_NAME=$service_name -e TEST_ID=$TEST_ID -e COMPILATION_MODE=$compilation
}

SERVICES=$(yq e '.services[] | select(.protocol == "http")' config.yaml)
