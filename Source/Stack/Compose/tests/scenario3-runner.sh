...
SCENARIO_FILE="scripts/scenario3.js"
...
run_k6_test() {
    ...
    k6 run --out influxdb=$INFLUXDB_WRITE_URL $SCENARIO_FILE -e SERVICE_URL=$url -e TEST_ID=$TEST_ID -e COMPILATION_MODE=$compilation
}

echo "Starting tests for services"

# Loop over services and perform actions
jq -c '.services[] | select((.protocol == "http") and (.name == "BPS"))' config.json | while IFS= read -r service; do
