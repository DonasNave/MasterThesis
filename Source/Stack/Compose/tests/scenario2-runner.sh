...
run_k6_test() {
    ...
    k6 run --out influxdb=http://localhost:8086/k6 scripts/scenario2.js -e SERVICE_URL=$url -e TEST_ID=$TEST_ID -e COMPILATION_MODE=$compilation
}

SERVICES=$(yq e '.services[] | select(.protocol == "http" && .name == "FUS")' config.yaml)
